...
// const extensionAPI = typeof browser === "undefined" ? chrome : browser; - non-userscript

// commenting out anything that won't work in a userscript - will have a note
...
async function commonFunctionGetSiteDataByDestination() {
  ...
  for (let i = 0; i < LANGS.length; i++) {
    promises.push(fetch(extensionAPI.runtime.getURL('data/sites' + LANGS[i] + '.json')) // alex you said to either remake the chrome extension langauge for this or just remake JUST this singular one and have to deal with more extension files
      .then((resp) => resp.json())
      .then((jsonData) => {
